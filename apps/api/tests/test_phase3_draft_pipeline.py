...
async def test_draft_pipeline_proces_lead_and_step(fake_llm):
    ...
    async with factory() as session, session.begin():
        await set_tenant_for_session(session, str(tenant_id))
        svc = DraftService(session, llm=fake_llm)
...
async def test_draft_idempotent_within_same_lead_step(fake_llm):
...
async def test_force_regenerate_replaces_draft(fake_llm):
    factory = get_session_factory()
    tenant_id = uuid.uuid4()
    from outreach_os.domain.models.campaign import Campaign
    from outreach_os.domain.models.campaign_step import CampaignStep
...
async def test_draft_status_filter_and_pagination_via_api(client):
