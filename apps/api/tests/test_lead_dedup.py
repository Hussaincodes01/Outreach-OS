...
async def test_db_unique_index_rejects_direct_duplicates():
    ...
    async with factory() as session:
        # NOT combinable with the `session.begin()` below: the first
        # transaction has to COMMIT before the duplicate insert opens a second
        # one, which is the race this test reproduces. noqa: SIM117
        ...
        with pytest.raises(Exception, match=r"(?i)uq_lead_tenant_email|duplicate"):
